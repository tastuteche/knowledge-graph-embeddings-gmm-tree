...
def load_kge_vectors(fname):
    """
    Loads a kge vectors file, returns a DataFrame.
    """
    vocab = []
    vectors = None
...
class Tree:
    ...
    def _print_tree(self, node):
        if(node != None):
            ...
            #print(str(node.vecs.index) + ' ' + str(node.code))
    ...
    def _gen_G(self, node, G, father=None):
        if(node != None):
            self._gen_G(node.l, G, father=node)
            #print(str(node.vecs.index) + ' ' + str(node.code))
            if father != None:
                u = '\n'.join(sorted(father.vecs.index.tolist()))
            else:
                u = 'ROOT'
            v = '\n'.join(sorted(node.vecs.index.tolist()))
            G.add_edge(u, v)
            self._gen_G(node.r, G, father=node)
...
def cluster_next_node(t, node, node_codes, verbose=0):
    ...
    if len(node.vecs) < 2:
        ...
        t.add(vecs=node.vecs.iloc[0:1], node=node,
              side=0, mean=node.vecs.iloc[0])
        t.add(vecs=node.vecs.iloc[1:2], node=node,
              side=1, mean=node.vecs.iloc[1])
...
def gmm_clustering(vec_file, save_file, verbose=0):
    ...
    while not is_clustering_finished(terminals):
        ...
        for next_terminal in terminals:
            t, node_codes = cluster_next_node(t, t.get_node(
                next_terminal['code']), node_codes, verbose=verbose)
...
if __name__ == '__main__':
    gmm_clustering('E.txt', 'E.dot', verbose=0)
    # dot -Tpng -Kdot  G.dot -o G.png
